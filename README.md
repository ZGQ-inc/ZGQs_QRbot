...
1. 打开 `QRSpeed` ，输入你的**小号**的 `账号`  `密码` ，勾选 `记住密码`  `自动登录` ，点击 `登入` 。

2. 转到 `插件列表` ，勾选 `QRDICPro` 。

3. 长按 `QRDICPro` 一栏，勾选 `授权skey`  `授权pskey` 。

4. **为防止进程被系统误杀**，点击 `后台权限` ，点击 `电池优化白名单` ，在弹出的对话框点击 `确认` 。

5. 返回，点击 `QRDICPro` 一栏，设置 `主人` 为自己的**大号**。
...
7. 同理返回到主界面，点击左上角 `侧边栏` ，点击 `后台权限` ，点击 `电池优化白名单` ，在弹出的对话框点击 `确认` 。

8. 返回，转到 `群列表` ，勾选需要启用的群。
